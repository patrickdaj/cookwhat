...
// A full browser-like header set. Some recipe sites (Serious Eats, Bon Appétit,
// NYT, Epicurious) sit behind bot protection that blocks datacenter IPs — those
// will 403 from a cloud/CI environment no matter what, but work when you run
// `cookwhat recipe fetch` from your own machine. These headers maximize the
...
export async function fetchRecipe(url) {
  ...
  if (res.status === 403 || res.status === 429) {
    throw new Error(
      ...
      `This commonly happens from cloud/CI IPs — try running ` +
      `\`cookwhat recipe fetch\` locally, or paste the recipe and save it manually.`
    );
  }
